...
import { GetCollectionEntry, UpdateCollectionEntry } from "../../../utils/apiCalls";
...
const CollectionEntry = () => {
   ...
   const [isCreateCollectionEntry, setIsCreateCollectionEntry] = useState(false);
   const [collectionEntryData, setCollectionEntryData] = useState(initialState);
   ...
   useEffect(() => {
      if (userAuthData) {
         const CollectionEntryPermission = userAuthData?.permissions?.find(
            (val) => val?.MilkCollection
         );
         setPermission(CollectionEntryPermission?.MilkCollection);
      }
   }, []);
   ...
   const getCollectionEntry = () => {
      setIsLoading(true); // Show the loading spinner
      GetCollectionEntry((res) => {
         let { status, data, message } = res;
         if (status === 200) {
            setCollectionEntryData(data);
         ...
         UpdateCollectionEntry(
            (res) => {
               ...
               if (status === 200) {
                  ...
                  setIsCreateCollectionEntry(!isCreateCollectionEntry);
                  setShowConfirmModal1(true);
               } else if (status === 403) {
                  setShowConfirmModal(false);
                  setAlertText("You don't have access to perform this operation");
   ...
   return (
      <>
         ...
         >
            <CModalHeader>
               <CModalTitle>Generated Collection Entry Password</CModalTitle>
            </CModalHeader>
