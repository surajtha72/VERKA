...
import "./DateWiseReconcillationCycleList.scss";
...
const ReconcillationCycleList = () => {
  ...
  const [isCycleWise, setisCycleWise] = useState(false);
    ...
    items?.push({
      ...
      heading_2: (
        <div style={{ display: "flex", flexDirection: "column" }}>
          <Link
            to={isddSelected ? `/datewise-reconcillation-report` : null}
            style={{
          ...
          >
            ...
            >
              Generate Date-Wise Reconcillation Report
            </span>
          </Link>
        </div>
      ),
    ...
    shiftWiseItems.push(
      {
        heading_2: (
          <div style={{ display: "flex", flexDirection: "column" }}>
            <Link
              to={(startDate !== null && endDate !== null && shift !== null) ? `/bmc-collection-report` : null}
              style={{
      ...
      {token ? <div className="reconcillation">
        <div className="reconcillation__container">
          <div className="reconcillation__header">
            <div className="reconcillation__header__section">
              <div className="reconcillation__header__section__main">
                <h5>Company: Verka</h5>
                <h4>{`${"Date-Wise Reconcillation Report"}`}</h4>
              </div>
          ...
          <div className="reconcillation__table">
            ...
            {
              isCycleWise ? <button onClick={() => { setisCycleWise(isCycleWise ? false : true) }} className="buttons">Get Cycle-Wise Reports</button>
                :
                <button onClick={() => { setisCycleWise(isCycleWise ? false : true) }} className="buttons">Get Shift-Wise Reports</button>
            }
            <div
            ...
            >
              {
                isCycleWise ? <CTable columns={shiftWisecolumns} items={shiftWiseItems} hover className="striped-table" />
                  :
...
export default ReconcillationCycleList;