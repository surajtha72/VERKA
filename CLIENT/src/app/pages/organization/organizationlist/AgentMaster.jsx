import {
  ...
  CTable,
  CFormSelect,
    CFormCheck,      // ⬅️ add this

} from "@coreui/react";
import React, { useEffect, useState } from "react";
import "./AgentMaster.scss";
import {
  CreateOrganization,
  DeleteOrganization,
  GetOrganization,
...
const initialState = {
  ...
  accountNumber: "",
  ifscCode: "",
   allowCan: false,          // ⬅️ new
  milkCollectionUom: "",    // ⬅️ new
  milkDispatchUom: "",  
};
const AgentList = () => {
  ...
  const filterTableData = () => {
    if (searchTerm === "") {
      setFilteredData(allOrganizationTableData);
    } else {
  ...
  {
    allOrganizationTableData?.map((val, ind) => {
      excelitem.push({
        SlNo: ind + 1,
        AgentCode: val?.id,
        ...
        AccountHolderName: val?.accHolderName ? val?.accHolderName : " ",
        Commission: val?.commission ? val?.commission : " ",
        HeadLoad: val?.headload ? val?.headload : " ",
  ...
  const pageIndex = activeNewPage - 1 ? activeNewPage - 1 : activePage - 1;
  {
    filteredData?.map((val, ind) => {
      ...
      items.push({
        ...
        Commission: val?.commission ? val?.commission : " ",
        HeadLoad: val?.headload ? val?.headload : " ",
        IfscCode: val?.ifscCode ? val?.ifscCode : ' ',
        Actions: (
          <div
            style={{
              display: "flex",
  ...
  useEffect(() => {
    getOrganizationData();
  }, [activeNewPage, newPageSize, totalRecordsCount, orgUnitType]);
  ...
  const validateFields = () => {
    ...
    if (!organizationData.phoneNumber) {
      errObj.phoneNumber = "This Field is Required";
    } else {
      errObj.phoneNumber = "";
    }
    ...
    if (!organizationData.accountNumber) {
      errObj.accountNumber = "This Field is Required";
    } else {
      errObj.accountNumber = "";
    }
    if (!organizationData.accountHolderName) {
      errObj.accountHolderName = "This Field is Required";
    } else {
      errObj.accountHolderName = "";
    }
    if (!organizationData.ifscCode) {
    ...
    if (!isButtonDisabled) {
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        const payload = {
          ...
          accHolderName: organizationData?.accountHolderName,
          ifscCode: organizationData?.ifscCode == '' ? null : organizationData?.ifscCode ,
          ...
          enforceNoDueCollection:
            selectedOption === "3"
              ? organizationData?.enforceNoDueCollection === "1"
                ...
                : false
              : null,
              allowCan: organizationData?.allowCan,                      // ⬅️ new
  milkCollectionUom: organizationData?.milkCollectionUom,    // ⬅️ new
  milkDispatchUom: organizationData?.milkDispatchUom,
        };
  ...
  const handleEdit = (id) => {
    ...
    setIsCreateOrganization(!isCreateOrganization);
    const payload = {
      id: allOrganizationTableData?.find((org) => org.id === id)?.id,
    };
    if (
      allOrganizationTableData?.find((org) => org.id === id)
        ?.organizationType === 3
    ) {
      setSelectedOption("3");
    } else {
    ...
    console.log(allOrganizationTableData?.find((org) => org.id === id));
    setOrganizationData(
      {
        outletType:
          allOrganizationTableData?.find((org) => org.id === id)
            ?.organizationType || "",
        parentId:
          allOrganizationTableData?.find((org) => org.id === id)?.parentId || "",
        name: allOrganizationTableData?.find((org) => org.id === id)?.name || "",
        businessRegnNo:
          allOrganizationTableData?.find((org) => org.id === id)
            ?.businessRegnNo || "",
        gstNo:
          allOrganizationTableData?.find((org) => org.id === id)?.gstNo || "",
        addressLine1:
          allOrganizationTableData?.find((org) => org.id === id)?.addressLine1 ||
          "",
        addressLine2:
          allOrganizationTableData?.find((org) => org.id === id)?.addressLine2 ||
          "",
        vctId:
          allOrganizationTableData?.find((org) => org.id === id)?.vctId || "",
        state:
          allOrganizationTableData?.find((org) => org.id === id)?.stateId || "",
        district:
          allOrganizationTableData?.find((org) => org.id === id)?.districtId ||
          "",
        geocode:
          allOrganizationTableData?.find((org) => org.id === id)?.geocode || "",
        phoneNumber:
          allOrganizationTableData?.find((org) => org.id === id)?.phoneNumber || "",
        headload:
          allOrganizationTableData?.find((org) => org.id === id)?.headload ||
          0.0,
        commission:
          allOrganizationTableData?.find((org) => org.id === id)?.commission ||
          0.0,
        capacity:
          allOrganizationTableData?.find((org) => org.id === id)?.capacity || "",
        morningShiftStartTime:
          allOrganizationTableData?.find((org) => org.id === id)
            ?.morningShiftStartTime || "",
        morningShiftEndTime:
        ...
        accountNumber: allOrganizationTableData?.find((org) => org.id == id)?.accountNumber || "",
        accountHolderName: allOrganizationTableData?.find((org) => org.id == id)?.accHolderName || "",
        ifscCode: allOrganizationTableData?.find((org) => org.id == id)?.ifscCode || "",
        payrollTypes:
          allOrganizationTableData?.find((org) => org.id === id)
            ?.payrollTypeId || "",
        enforceStrictTiming:
        ...
        enforceNoDueCollection:
          allOrganizationTableData?.find((org) => org.id === id)
            ...
            : 2 || "",
            allowCan:
    allOrganizationTableData?.find((org) => org.id === id)?.allowCan || false, // ⬅️ new
  milkCollectionUom:
    allOrganizationTableData?.find((org) => org.id === id)
      ?.milkCollectionUom || "",                                              // ⬅️ new
  milkDispatchUom:
    ...
    setIsId(allOrganizationTableData?.find((org) => org.id === id)?.id);
  };
  console.log('userData: ', organizationData);
        ...
        <div className="agent">
          <div className="agent__container">
            <div className="agent__header">
              <div className="agent__header__section">
                <div className="agent__header__section__main">
                  <h5>Company: Verka</h5>
                  <h4>{`UAM - ${isCreateOrganization
                    ? isId
                      ? "Edit Agent"
                      : "Create Agent"
                    : "Agent"
                ...
                <div className="agent__header__section__bottom">
                  <Header />
                </div>
              </div>
            </div>
                        ...
                        <CForm>
                          <CRow>
                            ...
                            <CCol lg={4} md={2} sm={3}>
                              ...
                              />
                              <p style={{ color: "red", fontSize: "x-small" }}>
                                {organizationDataErr.phoneNumber}
                              </p>
                            </CCol>
                            {/* <CCol lg={4} md={2} sm={3}>
                                <CFormLabel htmlFor="nf-email">
                            ...
                            <CCol lg={4} md={2} sm={3}>
                              ...
                              />
                              <p style={{ color: "red", fontSize: "x-small" }}>
                                {organizationDataErr.accountNumber}
                              </p>
                            </CCol>
                            <CCol lg={4} md={2} sm={3}>
                              <CFormLabel htmlFor="nf-email">
                              ...
                              <CFormInput
                                ...
                                value={
                                  organizationData.accountHolderName
                                }
                                onChange={handleInputChange}
                                placeholder="Enter Account Holder Name.."
                              />
                              <p style={{ color: "red", fontSize: "x-small" }}>
                                {organizationDataErr.accountHolderName}
                              </p>
                            </CCol>
                            <CCol lg={4} md={2} sm={3}>
                              <CFormLabel>
                              ...
                              <CFormInput
                                ...
                                size="sm"
                                name="ifscCode"
                                value={organizationData.ifscCode}
                                maxLength={15}
                                onChange={handleInputChange}
                          ...
                          <CCol lg={4} md={2} sm={3}>
  <CFormLabel htmlFor="allowCan">
    Allow Can?
  </CFormLabel>
  <CFormCheck
    id="allowCan"
    checked={organizationData.allowCan}
    onChange={(e) =>
      setOrganizationData((prev) => ({
        ...prev,
        allowCan: e.target.checked,
      }))
    }
  />
</CCol>
...
<CCol lg={4} md={2} sm={3}>
  <CFormLabel htmlFor="milkCollectionUom">
    Milk Collection UOM
  </CFormLabel>
  <CFormSelect
    id="milkCollectionUom"
    size="sm"
    value={organizationData.milkCollectionUom}
    onChange={(e) =>
      handleDropDown("milkCollectionUom", e.target.value)
    }
  >
  ...
  <p style={{ color: "red", fontSize: "x-small" }}>
    {organizationDataErr.milkCollectionUom}
  </p>
</CCol>

<CCol lg={4} md={2} sm={3}>
  <CFormLabel htmlFor="milkDispatchUom">
    Milk Dispatch UOM
  </CFormLabel>
  <CFormSelect
    id="milkDispatchUom"
    size="sm"
    value={organizationData.milkDispatchUom}
    onChange={(e) =>
      handleDropDown("milkDispatchUom", e.target.value)
    }
  >
    <option value="">Select UOM</option>
    <option value="KG">KG</option>
  ...
  <p style={{ color: "red", fontSize: "x-small" }}>
    {organizationDataErr.milkDispatchUom}
  </p>
</CCol>
              ...
              <>
                <div className="agent__table">
                  <div className="agent__table__header">
                    <div className="agent__table__header__section">
                      <input
                        type="text"
                  ...
                  >
                    <div className="agent__table__pagination__container">
                      <select
                        value={newPageSize}
                        onChange={(e) =>
