...

   {
      filteredData?.map((val, ind) => {
         items.push({
            SlNo: ind + 1,
            id: val?.id,
            heading_1: val?.vehicleNo ? val?.vehicleNo.RegistrationNo : " ",
            heading_2: val?.productCategory ? val?.productCategory.CategoryName : " ",
      ...
      const handleEdit = (id) => {
         ...
         const productCategId = sampleTableData.find((role) => role.id === id)?.productCategory.Id;
         setSelectedCategoryId(productCategId);

         setSampleData(
            {
               vehicleNo: sampleTableData.find((sample) => sample.id === id)?.vehicleNo.Id || "",
               productCategory: sampleTableData.find((sample) => sample.id === id)?.productCategory.Id || "",
               productName: sampleTableData.find((sample) => sample.id === id)?.productName || "",
               // routeId: sampleTableData.find((sample) => sample.id === id)?.routeId.Id || "",
               temperature: sampleTableData.find((sample) => sample.id === id)?.temperature || "",
               ...
               protein: sampleTableData.find((sample) => sample.id === id)?.protein || "",
               lactose: sampleTableData.find((sample) => sample.id === id)?.lactose || "",
               salt: sampleTableData.find((sample) => sample.id === id)?.salt || "",
               water: sampleTableData.find((sample) => sample.id === id)?.water || "",
               analyst: sampleTableData.find((sample) => sample.id === id)?.analyst || "",
               status: sampleTableData.find((sample) => sample.id === id)?.status || "",
               remarks: sampleTableData.find((sample) => sample.id === id)?.remarks || "",
               sampledBy: sampleTableData.find((sample) => sample.id === id)?.sampledBy || "",
            },
            payload
         );
      ...
      const handleCheckAutomaticAcido = () => {
         const checkBox = document.getElementById("automaticAcido");
         setAutomaticAcidoChecked(!automaticAcidoChecked);
         setSampleInputDisable(true);
      }

      const handleCheckManualAcido = () => {
         // console.log("handle check manual called");
         const checkBox = document.getElementById("manualAcido");
         setSampleInputDisable(false);
         setAutomaticAcidoChecked(false);
         // setShowModal(true);
         // console.log(showModal);
      }

      const handleCheckAutomaticMilko = () => {
         const checkBox = document.getElementById("automaticMilko");
         setAutomaticMilkoChecked(!automaticMilkoChecked);
         setSampleInputDisable(true);
      }

      const handleCheckManualMilko = () => {
         // console.log("handle check manual called");
         const checkBox = document.getElementById("manualMilko");
         setSampleInputDisable(false);
         setAutomaticMilkoChecked(false);
         // setShowModal(true);
         // console.log(showModal);
      }
      ...
      //         setMeasuredValues(result.data);
      //         console.log(measuredValues)
      //         const arr = measuredValues.split(' ');
      //         setSampleData((prev) => ({ ...prev, ['temperature']: arr[0] }));
      //         setSampleData((prev) => ({ ...prev, ['fat']: arr[1] }));
      //         setSampleData((prev) => ({ ...prev, ['snf']: arr[2] }));
      //     })
      // }

      useEffect(() => {
         handleDeclaredValues()
      }, [selectedVehicleId])
      const handleDeclaredValues = () => {
         GetCSTVehicleDropdown((res) => {
            if (res.status == 200) {
               setSampleData(res.data[0]);
               setSampleData((prev) => ({ ...prev, "vehicleNo": selectedVehicleId || null }));
               setSampleData((prev) => ({ ...prev, "declaredFat": res.data[0]?.endFat || null }));
               setSampleData((prev) => ({ ...prev, "declaredSnf": (res.data[0]?.endSnf)?.toFixed(1) || null }));
               setSampleData((prev) => ({ ...prev, "declaredClr": res.data[0]?.endClr || null }));
            }
         }, selectedVehicleId)

         GetWBVehicleDropdown((result) => {
            if (result.status == 200) {
               // setSampleData((prev) => ({ ...prev, ["routeId"]: result?.data[0]?.routeId?.Id || null }));
               setSampleData((prev) => ({ ...prev, ["productCategory"]: result.data[0]?.productCategory?.Id || null }));
               setSampleData((prev) => ({ ...prev, ["productName"]: result.data[0]?.productName }));
               setSelectedCategoryId(result.data[0]?.productCategory.Id)
            }
         }, selectedVehicleId)
      }
      ...
      const autoMilkData = () => {
         GetWeighBridgeData((result) => {
            ...
            const regex = /\$?\s*LE\d*\.?\d*\|A\|CH\d*\.?\d*\|F(?<FAT>\d*\.?\d*)\|S(?<SNF>\d*\.?\d*)\|C(?<CLR>\d*\.?\d*)\|P(?<PROTEIN>\d*\.?\d*)\|L(?<LACTOSE>\d*\.?\d*)\|s(?<SALT>\d*\.?\d*)\|W(?<WATER>\d*\.?\d*)\|T(?<TEMPERATURE>\d*\.?\d*)?\s*\&/;
            ...
            setSampleData((prev) => ({ ...prev, 'protein': extractValues?.groups.PROTEIN }));
            setSampleData((prev) => ({ ...prev, 'lactose': extractValues?.groups.LACTOSE }));
            setSampleData((prev) => ({ ...prev, 'salt': extractValues?.groups.SALT }));
            setSampleData((prev) => ({ ...prev, 'salt': extractValues?.groups.WATER }));
            setSampleData((prev) => ({ ...prev, 'temperature': extractValues?.groups.TEMPERATURE }));
      ...
      const getWaitingVehicles = () => {
         ...
         GetWaitingVehicles((res) => {
            let { status, data, message } = res;
            setWaitingVehiclesData(data)
            let len = 0;
            data?.forEach(item => {
               // console.log(item)
               if (item.tareWeight === null) {
                  vehicleNo.push(item.vehicleNo)
                  len++;
               }
            })
            setWaitingVehiclesCount(vehicleNo.length)
         }, testDate)
      }
                        ...
                        <Paper elevation={3}>
                           <div className="container">
                              <div>
                                 <CForm method="post" onSubmit="">
                                    <CRow>
                                       <CCol lg={12}><p style={{ fontWeight: "bold" }}>Statistics :</p></CCol>
                                       <CCol lg={3}>
                                    ...
                                    {/* <CRow>
                                                ...
                                                <CCol lg={4}>
                                                    ...
                                                    <CFormSelect
                                                        size="sm"
                                                        value={sampleData.weighbridgeNo}
                                                        name="weighbridgeNo"
                                                        onChange={(e) => {
                                    ...
                                    <CRow>
                                       ...
                                       <CCol lg={4}>
                                          ...
                                          <CFormSelect
                                             ...
                                             onChange={(e) => {
                                                ...
                                                if (selectedVehicleId != 0) {
                                                   ...
                                                   setSampleData((prev) => ({ ...prev, "vehicleNo": selectedVehicleId || '' }));
                                                }
                                          ...
                                          >
                                             ...
                                             {waitingVehiclesData?.length &&
                                                waitingVehiclesData?.map(
                                                   (option, index) => {
                                                      return (
                                                         <option key={index} value={option.vehicleNo.Id}>
                                                            {option.vehicleNo.RegistrationNo}
                                                         </option>
                                                      );
                                                   }
                                                )}
                                       ...
                                       {/* <CCol lg={4}>
                                          ...
                                          <Select
                                             size="sm"
                                             styles={{ control: (base, _state) => ({ ...base, minHeight: '35px', height: '35px' }) }}
                                             options={routeMasterData}
                                             value={routeMasterData.find(
                                                (option) =>
                                                   option.id === sampleData.routeId)}
                                             name="routeId"
                                             isSeachable
                                    ...
                                    <CRow>
                                       <hr />
                                       {/* <CCol lg={2}>Acido Meter :</CCol>
                                                    <CCol lg={2}>
                                                        <CFormCheck id="automaticAcido" label="Automatic" onClick={handleCheckAutomaticAcido} defaultChecked={automaticAcidoChecked} />
                                                    </CCol>
                                                    <CCol lg={2}>
                                                        <CFormCheck id="manualAcido" label="Manual" onClick={handleCheckManualAcido} defaultChecked={!automaticAcidoChecked} />
                                                    </CCol>
                                                    <CCol lg={6}>
                                                        <CButton style={{
                                                            border: 0,
                                       ...
                                       <CCol lg={2}>
                                          <CFormCheck id="automatic" label="Aautomatic" onClick={handleCheckAutomatic} defaultChecked={automaticChecked} />
                                       </CCol>
                                       <CCol lg={2}>
                                    ...
                                    <CRow>
                                       <CCol lg={2}><p style={{ fontWeight: "bold" }}>Parameter: </p></CCol>
                                       <CCol lg={4}>Declared Values: </CCol>
                                       <CCol lg={6}>Measured Values:{" "}
                                          <span style={{ color: "red" }}>*</span></CCol>
                                    </CRow>
                                    <CRow>
                                       <CCol lg={2}>Fat%{" "}
                                          <span style={{ color: "red" }}>*</span>
                                    ...
                                    <CRow>
                                       <CCol lg={2}>Snf%<span style={{ color: "red" }}>*</span></CCol>
                                       <CCol lg={4}>
                                          <CFormInput
                                             size="sm"
                                             type="Name"
                                    ...
                                    <CRow>
                                       <CCol lg={2}>CLR<span style={{ color: "red" }}>*</span></CCol>
                                       <CCol lg={4}>
                                          <CFormInput
                                             size="sm"
                                             type="Name"
                                    ...
                                    <CRow>
                                       ...
                                       {/* <CCol lg={4}>
                                                        <CFormLabel htmlFor="nf-email">
                                                            Sampled By<span style={{ color: "red" }}>*</span>
                                                        </CFormLabel>
                                                        <CFormInput
                                                            size="sm"
                                                            type="Name"
                                                            name="sampledBy"
                                                            value={sampleData.sampledBy}
                                                            onChange={handleInput}
                                                            placeholder="Enter Sampled By.."
                                                            aria-label="default input example"
                                                            onInput={(e) => {
                                                                e.target.value = e.target.value.replace(
                                                                    /[^a-zA-Z\s]/g,
