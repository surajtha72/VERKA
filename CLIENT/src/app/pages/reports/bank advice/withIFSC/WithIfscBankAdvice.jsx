...
import "./WithIfscBankAdvice.scss";
import { GetBill, GetBillBmc, GetBmcBankAdvice, GetCurrentIncentiveMaster, GetIncentiveMaster, GetIncentiveSlab, GetPreviousMilkData, GetPreviousMilkQuantity, GetProductSalesToAgent } from "../../../../utils/apiCalls";
...
const BmcBankAdvice = () => {
    ...
    const handleCycle = () => {
        navigate("/bmc-cycle-list-bank-advice-ifsc");
    };
    ...
    const generatePDF = () => {
        const invoiceElements = document.querySelectorAll(".adviceifsc");
        const htmlContents = [];
    ...
    function inWords(num) {
        ...
        str += (n[5] != 0) ? ((str != '') ? '' : '') + (a[Number(n[5])] || b[n[5][0]] + ' ' + a[n[5][1]]) + 'Rupees And Zero Paise Only ' : '';
        return str;
    }
        ...
        <>
            {token ? (
                ...
                >
                    ...
                    <div style={{ width: '2vw', height: '2vw', position: 'absolute', right: '1.2vw', top: '4vw' }}>
                    ...
                    {billDetails ? (
                        <div><img
                            ...
                            className="download"
                        />
                            <CButton className="export" onClick={handleExportToExcel}>Export To Excel</CButton></div>
                    ) : (
                        ""
                    )}
                    ...
                    {isLoading ? (
                        ...
                        <div ref={bankAdviceRef}>
                            <div className="adviceifsc">
                                <div className="adviceifsc__header">
                                    <div className="adviceifsc__header__heading">
                                        <p>Verka - Punjab Milk Producers Federation & Cooperative Society</p>
                                    </div>
                                    <div className="adviceifsc__header__bottomheading">
                                        <p>Bank Advice &nbsp;
                                            From&nbsp;&nbsp;{startDt}
                                            &nbsp;&nbsp;To&nbsp;&nbsp;{endDt} &nbsp;&nbsp;</p>
                                        <p>Cycle No. &nbsp;&nbsp; Page No. 1</p>
                                    </div>
                                    <div className="adviceifsc__header__secondbottomheading">
                                        <p>1 State Bank of India</p>
                                        <p>P.Date:&nbsp;&nbsp;{moment(new Date()).format("YYYY/MM/DD")}</p>
                                    </div>
                                </div>
                                <div className="adviceifsc__table__header">
                                ...
                                {billDetails
                                    .filter(val => val.organization?.ifscCode !== null)
                                    .map((val, ind) => {
                                        if (val.amount > 1000) {
                                            branchTotal += parseFloat((val.amount).toFixed(0));
                                            return (
                                                <div className="adviceifsc__table__content" key={ind}>
                                                    {/* {console.log("val-->", val)} */}
                                                    <p style={{ width: "10%" }}>{ind + 1}</p>
                                                    <p>{val.organization.accHolderName}</p>
                                                    <p>{val.organization?.ifscCode}</p>
                                ...
                                <div className="adviceifsc__table__footer">
                                    <p>Branch Total : </p>

                                    <p style={{ fontWeight: 550 }}>{branchTotal.toFixed(0)}</p>
                                </div>
                                <div style={{ alignItems: "flex-end" }}>
                                    <p style={{ fontWeight: 550, alignItems: "end" }}>{inWords(parseFloat(branchTotal.toFixed(0)))}</p>
                                    <br />
                                </div>
                                <div className="adviceifsc__footer">
                                    <p>FOR, GANGA DAIRY LIMITED
                                        <br /><br /><br />Authorized Signatory</p>
                                </div>
                            </div>
